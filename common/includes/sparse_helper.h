...
struct SparseMatrix {
  ...
  std::vector<int> CSRColIndex;
  std::vector<float> CSRVal;
  std::vector<int> CSCColPtr;
  std::vector<int> CSCRowIndex;
  std::vector<float> CSCVal;
  int M;
  int K;
  int nnz;
  int M_CSC;
...
inline int cmp_by_row_column(const void *aa,
                             const void *bb) {
  rcv *a = (rcv *) aa;
  rcv *b = (rcv *) bb;
  if (a->r > b->r) return +1;
...
inline int cmp_by_column_row(const void *aa,
                             const void *bb) {
  rcv *a = (rcv *) aa;
  rcv *b = (rcv *) bb;
...
inline void sort_by_fn(int nnz_s,
...
inline void mm_init_read(FILE *f,
                         char *filename,
                         MM_typecode &matcode,
                         int &m,
                         int &n,
...
inline void load_S_matrix(FILE *f_A,
                          int nnz_mmio,
                          int &nnz,
                          vector<int> &cooRowIndex,
                          vector<int> &cooColIndex,
  ...
  for (int i = 0; i < nnz_mmio; ++i) {
    ...
    auto *tmpPointer_v = reinterpret_cast<unsigned int*>(&value);;
...
inline void read_suitsparse_matrix(char *filename_A,
                                   vector<int> &elePtr,
                                   vector<int> &eleIndex,
                                   vector<float> &eleVal,
                                   int &M,
  ...
  if ((f_A = fopen(filename_A, "r")) == nullptr) {
...
inline void cpu_spmm_CSR(const int M,
                         const int N,
                         const int K,
                         const int NNZ,
                         const float ALPHA,
...
inline void generate_edge_list_for_one_PE(
  ...
  int e_dst, e_src;
  float e_attr;
  for (const auto &pp : tmp_edge_list) {
    e_src = pp.col - base_col_index;
    e_dst = pp.row / NUM_PE; //e_dst = tmp_edge_list[pp].row;
    e_attr = pp.attr;
...
inline void generate_edge_list_for_all_PEs(
