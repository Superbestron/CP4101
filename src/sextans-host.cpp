...
#include "../common/includes/mmio.h"
#include "sextans.h"
#include "../common/includes/sparse_helper.h"
...
int main(int argc, char **argv) {
  ...
  // TODO ZHEN: i feel that this 16 is related to NUM_CH_SPARSE

  // ZHEN: arrounding the number of columns to a multiple of 16
  int edge_list_ptr_fpga_size = ((edge_list_ptr.size() + 15) / 16) * 16;

  // ZHEN: arrounding the number of columns to a multiple of 1024
  ...
  //ZHEN: x2 because two output per PE?
  int CH_SPARSEx2 = NUM_CH_SPARSE * 2;
  int mat_C_fpga_column_size = ((M + CH_SPARSEx2 - 1) / CH_SPARSEx2) * CH_SPARSEx2;
  ...
  cout << "launch kernel\n";
  double time_taken = tapa::invoke(Sextans, bitstream,
                                   tapa::read_only_mmap<int>(edge_list_ptr_fpga),
                                   tapa::read_only_mmaps<unsigned long, NUM_CH_SPARSE>(sparse_A_fpga_vec).reinterpret
                                       < ap_uint < 512 >> (),
  ...
  //         int pos = mat_C_fpga_column_size * (nn / 8) + (mm / NUM_CH_C) * NUM_CH_C + nn % 8;
  //         float v_fpga = mat_C_fpga_vec[mm % NUM_CH_C][pos];
